...
is.autosome <- chromosome(oligoSet) < 23
sds <- apply(cn[is.autosome, ], 2, mad, na.rm=TRUE)
sds <- matrix(sds, nrow(oligoSet), ncol(oligoSet), byrow=TRUE)
cnConfidence(oligoSet) <- sds
@
...
hmmOpts <- hmm.setup(oligoSet,
		     copynumberStates=log2(c(0, 1, 2, 2, 3)),
		     states=c("hom-del", "hem-del", "ROH", "normal", "amp"),
		     normalIndex=4,
		     log.initialP=rep(log(1/5), 5),
		     prGenotypeHomozygous=c(2/3, 0.99, 0.99, 0.7, 0.7))
@
...
<<findOverlaps>>=
mm <- findOverlaps(fit.van, oligoSet)
featureNames(oligoSet)[mm[,1]==2]
@
...
@ Note that the default in the above command would be to use a common
x- and y-scales for each panel.  To allow the x-axes to change for
each panel, one could set the x-scales to 'free':
<<multipanelSingeLocus,eval=FALSE>>=
xyplot(cn ~ x | id, oligoSet, range=RangedDataObject, ylim=c(-0.5,4), scales=list(x="free"),
       panel=xypanel)
@ The function \Rfunction{xypanel} provides default colors for
...
name \emph{range} to the \Rfunction{xyplot}. See Figure
\ref{fig:xy}. Note that the formula in the example below conditions on
\emph{range} instead of \emph{id}.  The conditioning variable for
displaying multiple panels of a single sample must be called
'range'. We plot a 2 Mb window surrounding each of the alterations in
the simulated data by specifying \texttt{frame=2e6}.
<<xyplotExample,fig=TRUE,include=FALSE, width=9, height=6>>=
...
xy.example <- xyplot(cn ~ x | range, oligoSet, range=ranges.altered, frame=2e6,
			...
			panel=xypanel, pch=21, border="grey")
@
See \texttt{?xyplot} for additional details.

<<xyplotPrint,echo=FALSE>>=
...
\begin{figure}[t]
  ...
  \caption{\label{fig:xy} The method \Rfunction{xyplot} is used to
    create a multi-panel display of alterations in a
    single sample. Each panel displays a single copy number alteration
...
cnSet <- cnSet[chromosome(cnSet) <= 2, ]
...
##trace(VanillaICE:::gtEmission, browser)
##tmp <- VanillaICE:::gtEmission(snpSet, hmmOpts)
